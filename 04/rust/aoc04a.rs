...
fn main() {
    ...
    for line in contents.lines() {
        ...
        // doesn't work..
        // let e1_set: HashSet<u16> = HashSet::from(e1_range);
        // let e2_set: HashSet<u16> = HashSet::from(e2_range);

        let e1_set:HashSet<u16> = e1_range.into_iter().collect();
        let e2_set:HashSet<u16> = e2_range.into_iter().collect();

        let e1_size = e1_set.len();
        let e2_size = e2_set.len();

        let intersection = e1_set.intersection(&e2_set).collect::<Vec<_>>();

        if intersection.len() == cmp::min(e1_size, e2_size) {
            counter = counter + 1;
        }
    }
