...
fn main() {
    ...
    for action in actions.iter() {
        ...
        // first, move the elements to a temporary store; crates will be "moved" here first and
        // then moved to their actual destination, to ensure the proper order
        ...
        // now, move from the temp store to the actual destination
        for _ in 1..=*num_items {
            //crates.get_mut(to).map(|val| val.push(item));
            let item = tmp_store.pop().unwrap();
