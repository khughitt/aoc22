...
function main()
    ...
    # how much free space is currently available?
    free_space = FS_TOTAL_SIZE - root.size

    # space needed?
    space_needed = UPDATE_SPACE_NEEDED - free_space

    # variables to keep track of directory that best meets our requirements
    best_dir_size = 0
    best_dir_size_diff = Inf


    # iterate over directories in tree and find the best match
    for n in collect(PreOrderDFS(root))
        if n.is_dir && n.size >= space_needed
            # how much larger is the directory size than what is needed?
            size_diff = n.size - space_needed

            if size_diff < best_dir_size_diff
                best_dir_size = n.size
                best_dir_size_diff = size_diff
            end
        end
    end

    println("Size of best match: ", best_dir_size)
end
